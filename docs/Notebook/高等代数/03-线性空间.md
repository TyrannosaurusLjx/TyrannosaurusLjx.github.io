...
## 基,维数
- $V$是数域$K$上的线性空间
- $V$的维数记作$\dim_K V$,或者$\dim V$,如果$V$是无限维的,记作$\dim V=\inft$,特别的$\dim \{\vec{0}\}=0$
...
#### **定理 1**
- 数域$K$上齐次线性方程组有解的时候,解空间$W$的维数满足:$\dim W = n-rank(A)$
...
#### U 的结构
- 设$\eta_1,\cdots,\eta_{n-r}$是对应齐次线性方程组$AX=B$的解空间$W$的一个基,则对应非齐次线性方程组的解为$\gamma_0+k_1\eta_1+\cdots+k_{n-r}\eta_{n-r}$,其中$\gamma_0$是特解,$k_1,\cdots,k_{n-r}\in K,r=rank(A)$
...
### 陪集
- 设$V$是数域$K$上的线性空间,$W\in subspace(V)$,定义$\beta\sim\alpha\iff\beta-\alpha\in W$,则$\sim$是$V$上的等价关系
  - 证明
...
#### 定理 1
- 设$V$是数域$K$上的线性空间,$W\in subspace(V)$,那么$\dim V/W = \dim V - \dim W$

#### 定理 2
- 若$V/W=span(\beta_1+W,\beta_2+W,\cdots,\beta_t+W)$,令$U=span(\beta_1,\beta_2,\cdots,\beta_t)$,则$V=W\oplus U$,且$(\beta_1,\beta_2,\cdots,\beta_t)$是$U$的一个基
